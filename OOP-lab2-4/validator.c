...
void validate(int zi, int suma, char tip[], char errors[][50], int* nr_err) {
	/*
	...
	*nr_err = 0;
	if (zi <= 0 || zi > 31) {
		strcpy_s(errors[*nr_err], 49, "Ziua trebuie sa fie intreg intre 1 si 31");
		(*nr_err)++;
	}
	if (suma <= 0) {
		strcpy_s(errors[*nr_err], 49, "Suma trebuie sa fie intreg strict pozitiv");
		(*nr_err)++;
	}
	if (strcmp(tip, "mancare") != 0 && strcmp(tip, "transport") != 0 && strcmp(tip, "telefon&internet") != 0 &&
		...
		strcpy_s(errors[*nr_err], 49, "Tipul trebuie sa fie: mancare, transport, telefon&internet, inbracaminte sau altele ");
		(*nr_err)++;
	}
}